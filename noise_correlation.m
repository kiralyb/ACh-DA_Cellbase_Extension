...
da_wind = [0.0 0.4];
    ...
    for i = 1:length(pairs)
        ...
        partition = find(tags2==1,1,'first');
        ...
        if isdoublepart
            [psth_a,spsth_a,~,tags,raszt_a] =  ultimate_psth(pairs{i,1},'trial','StimulusOn',ach_wind,'parts','#Type3','sigma',sigma_a);
            [psth_d,spsth_d,~,~,raszt_d] =  ultimate_psth(pairs{i,2},'trial','StimulusOn',da_wind,'parts','#Type3','sigma',sigma_d);
            for tag_inx= 1: length(tags)
                tags2(tag_inx) = str2double(tags{tag_inx}(end));
            end
            partition4Hz = find(tags2==1,1,'first');
            partition = find(tags2==2,1,'first');
            VE = loadcb(pairs{i,1},'TrialEvents');   % load events
